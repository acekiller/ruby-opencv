...
* Ruby 1.8.7, 1.9.3 and OpenCV 2.1 are supported.
...
==== Unix/Linux/Mac
  ...
  $ cd ruby-opencv
  $ git checkout master  # for OpenCV 2.3 or later. To use OpenCV 2.1, type "git checkout OpenCV_2.1" instead
...
For example, if you install OpenCV library to */opt/local/* like:
  ...
  $ cmake -D CMAKE_BUILD_TYPE=RELEASE -D CMAKE_INSTALL_PREFIX=/opt/local/ ./OpenCV-2.1.0
  $ make
  $ make install
